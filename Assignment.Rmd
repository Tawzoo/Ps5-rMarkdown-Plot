...
gap %>% 
  ...
  group_by(region, time) %>% 
  summarize(avg_LE = mean(lifeExpectancy)) %>% 
  mutate(prev = lag(avg_LE), growth = avg_LE - lag(avg_LE)) %>% 
  arrange(desc(growth))
```
Out of all continents, **Asia** and **Africa** had the highest growth in life expectancy while **Europe** had the least growth.
6. 
```{r}
gap %>% 
